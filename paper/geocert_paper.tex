...
% ===================== Simplified Conceptual Figure =====================
\begin{figure}[h!]
\centering
\begin{subfigure}[t]{0.32\textwidth}
\centering
\begin{tikzpicture}[node distance=1.2cm]
\node[draw,circle,minimum size=0.8cm,fill=blue!15] (C) {$C$};
\node[draw,circle,minimum size=0.8cm,fill=green!15,right=of C] (D) {$D$};
\node[draw,circle,minimum size=0.8cm,fill=orange!15,right=of D] (B) {$B$};
\draw[->,thick] (C) -- (D);
\draw[->,thick] (D) -- (B);
\node[below=0.5cm of D,font=\small] {\textbf{A.} Pipeline: $C \to D \to B$};
\end{tikzpicture}
\end{subfigure}\hfill
\begin{subfigure}[t]{0.32\textwidth}
\centering
...
\draw[thick,green] (0,1.2) -- (1.5,1.2);
\draw[->,thick] (0.75,1.0) -- (0.75,0.5);
\draw[thick,blue!50!green] (0,0.3) -- (1.5,0.3);
\node[right] at (1.6,1.35) {\tiny Convex blend};
\node[right] at (1.6,0.3) {\tiny (PSD-preserving)};

% Edge mix (broken)
\draw[thick,blue] (0,0) -- (0.7,0);
\draw[thick,green] (0.8,0) -- (1.5,0);
\draw[->,thick] (0.75,-0.2) -- (0.75,-0.7);
\draw[thick,blue!50!green] (0,-1.0) -- (0.7,-1.0);
\draw[thick,blue!50!green] (0.8,-1.0) -- (1.5,-1.0);
\node[right] at (1.6,-0.1) {\tiny Edge-mix};
\node[right] at (1.6,-1.0) {\tiny (breaks PSD)};

\node[below,font=\small] at (0.75,-1.4) {\textbf{B.} Blend types};
\end{tikzpicture}
\end{subfigure}\hfill
\begin{subfigure}[t]{0.32\textwidth}
\centering
...
\draw[thick,green] (-1,0) -- (-0.3,0);
\draw[thick,orange] (-0.3,0) -- (0.3,0);
\draw[thick,red] (0.3,0) -- (1,0);
\draw[->,very thick] (0,0) -- (0.5,-0.3);
\node[below] at (0,-0.5) {\tiny $\lambda_{\min}(B)$};
\node[below,font=\small] at (0,-0.9) {\textbf{C.} Consistency meter};
\end{tikzpicture}
\end{subfigure}
\caption{\textbf{Conceptual schematic.} \textbf{A:} GeoCert pipeline from similarities $C$ to distances $D$ to EDM Gram $B$. \textbf{B:} Convex blends preserve PSD structure; edge-mixing breaks it. \textbf{C:} Bottom eigenvalue $\lambda_{\min}(B)$ indicates consistency (negative = inconsistent).}
\label{fig:concept}
\end{figure}
% ======================================================================
