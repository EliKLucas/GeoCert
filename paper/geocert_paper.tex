...
\centering
\begin{tikzpicture}[>=Latex, node distance=1.6cm, scale=0.85, transform shape]
\node[draw,rounded corners,minimum width=1.6cm,minimum height=1.0cm,fill=blue!6] (C)
  {\shortstack{$C$\\\footnotesize (similarity)}};
\node[draw,rounded corners,minimum width=1.6cm,minimum height=1.0cm,fill=green!6,right=of C] (D)
  {\shortstack{$D$\\\footnotesize (distances)}};
\node[draw,rounded corners,minimum width=1.8cm,minimum height=1.0cm,fill=orange!6,right=of D] (B)
  {\shortstack{$B=-\tfrac12\,J D^{\circ2} J$\\\footnotesize (EDM Gram)}};
\draw[->,thick] (C) -- node[above,font=\tiny]{chordal map} (D);
\draw[->,thick] (D) -- node[above,font=\tiny]{double-center} (B);
\node[below=0.7cm of D,align=center,font=\small] {\textbf{A. Pipeline:} $C \to D \to B$};
\end{tikzpicture}
\end{subfigure}\hfill
\begin{subfigure}[t]{0.32\textwidth}
\centering
...
% Convex blend
\node[draw,rounded corners,fill=blue!6,minimum width=1.4cm,minimum height=0.6cm] (c1) {};
\node[draw,rounded corners,fill=green!6,minimum width=1.4cm,minimum height=0.6cm,below=0.1cm of c1] (c2) {};
\node[right=0.2cm of c1,align=left,font=\tiny] {Convex blend\\(smooth mix; PSD-preserving)};
\draw[->] ([xshift=0.7cm]c2.south) -- ++(0,-0.4cm);
\node[draw,rounded corners,fill=blue!10!green!20,minimum width=1.4cm,minimum height=0.6cm,below=0.8cm of c2] (cb) {};
% Edge mix
\node[draw,rounded corners,fill=blue!6,minimum width=1.4cm,minimum height=0.6cm,right=1.8cm of c1] (e1) {};
\node[draw,rounded corners,fill=green!6,minimum width=1.4cm,minimum height=0.6cm,below=0.1cm of e1] (e2) {};
\node[right=0.2cm of e1,align=left,font=\tiny] {Edge-mix\\(per-edge stitching; breaks PSD)};
\draw[->] ([xshift=0.7cm]e2.south) -- ++(0,-0.4cm);
\begin{scope}[shift={(2.8cm,-1.3cm)}]
  \draw[rounded corners,fill=blue!10!green!20] (0,0) rectangle (1.4,0.6);
  \draw[very thick,white] (0.0,0.3) -- (1.4,0.3);
  \draw[very thick,white] (0.7,0.0) -- (0.7,0.6);
\end{scope}
\node[below=0.9cm of cb,align=center,font=\small] {\textbf{B. Mix types:} convex blend vs.\ edge-mix};
\end{tikzpicture}
\end{subfigure}\hfill
\begin{subfigure}[t]{0.32\textwidth}
\centering
\begin{tikzpicture}[>=Latex, scale=0.85, transform shape]
% Gauge
\draw[very thick] (0,0) arc[start angle=180,end angle=360,radius=1.2];
\foreach \a in {180,225,270,315,360}{
  \draw[thin] ({1.2*cos(\a)},{1.2*sin(\a)}) -- ({1.35*cos(\a)},{1.35*sin(\a)});
}
% colored ticks (conceptual)
\draw[line width=3pt,green!60] (180:1.0) arc (180:230:1.0);
\draw[line width=3pt,orange!70] (230:1.0) arc (230:300:1.0);
\draw[line width=3pt,red!70] (300:1.0) arc (300:360:1.0);
% needle
\draw[very thick,->] (0,0) -- ({0.9*cos(310)},{0.9*sin(310)});
\node[below=1ex,font=\footnotesize] at (0,0) {$\lambda_{\min}(B)$};
\node[font=\small] at (0,-1.6) {\textbf{C. Consistency meter:} more negative $\Rightarrow$ less consistent};
\end{tikzpicture}
\end{subfigure}
